...
function checkbrute($user_id, $db) {
   ...
   if ($stmt = $db->prepare("SELECT date FROM userevents WHERE userId= ? AND eventType='password incorrect' AND date > '$valid_attempts'")) { 
...
function login_check($db) {
   ...
   if(isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['login_string'])) {
     ...
     if ($stmt = $db->prepare("SELECT passwordHash FROM login WHERE id = ? LIMIT 1")) { 
