<?php
  ...
  if ($action == 'update') {
    ...
    $editTimeStamp = time();
    $updateId = $_GET['id'];
    $sql = 'UPDATE articles SET shortname = ?, fullname = ?, tags = ?, content = ?,  description = ?, editUser = ?, editTimestamp = ? WHERE id = ?';
    $stmt = $db->prepare($sql);
    $stmt->bind_param('sssssssi', $shortName, $fullName, $tags, $content, $description, $editUser, $editTimeStamp, $updateId);
    $stmt->execute();
    $leave = 1;
    $id = $_GET['id'];
    ...
    $editUser = $_SESSION['username'];
    $editTimeStamp = time();
    $usersql = 'INSERT INTO articles (shortName,fullName,tags,content,  description, creationUser,creationtimeStamp) VALUES ( ? , ? , ? , ? , ? , ? , ? )';
    $userps = $db->prepare($usersql);
    $defaultRights = '000';
    $userps->bind_param('sssssss', $shortName, $fullName, $tags, $content, $description, $editUser, $editTimeStamp);
    $userps->execute();
...
<html xmlns="http://www.w3.org/1999/xhtml"> 
    <head> 
        ...
        <title>New-Growth - Edit Blog</title>
    ...
    <body>
        ...
        <div id='outterClear'>            
            <div id='clearMain' class='clearBox'>
                <div id='contentArea' class='insetBox fullContent'>
                  <h1><?php if ($action == 'new') { ?>New<?php } else if ($action == 'edit') { ?>Edit<?php } ?> Blog</h1>
                  <table class='editTable'>
                    <form method="POST" action='editBlog.php?action=<?php echo $futureAction; ?>' id='editForm' name='editForm'>
                    ...
                    <tr><td> </td><td> <form action="post_file.php" id="dropzone" class="dropzone"><div class="fallback"><input name="file" type="file" multiple /></div></form></td></tr>
        ...
        <script>
          ...
          var dropZoneTime = new Dropzone("#dropzone");
